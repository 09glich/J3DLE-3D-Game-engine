...
public class Window {
    ...
    private Event<MouseButton> MouseButtonDown;
    private Event<MouseButton> MouseButtonUp;
    ...
    public void WindowUpdate() {
        ...
        if (mousePosition != null) {
            previousMousePosition = mousePosition.copy();
        }
    ...
    public boolean getMouseKeyState(MouseButton button) {
        return GLFW.glfwGetMouseButton(window, button.keyint()) == GLFW.GLFW_PRESS;
    }
    public Vector2 getMousePosition() {
        return mousePosition;
    }
    public Vector2 getMouseDelta() {
        Vector2 mousePos = mousePosition.copy();
        return mousePos.sub(previousMousePosition);
    }
    ...
    public static class Cursor {
        public enum CursorVisibility {HIDDEN, VISIBLE}
        public enum CursorMode {LOCKED, LOCKEDCENTER, UNLOCKED}
        
        private static CursorVisibility visibilityState;
        private static CursorMode cursorMode;
        
        private static Long currentWindow = null;

        private static void updateCursor() {
            if (currentWindow == null) { currentWindow = App.getWindow().getWindowHandel(); }

            if (visibilityState == CursorVisibility.VISIBLE && cursorMode == CursorMode.UNLOCKED) {
                GLFW.glfwSetInputMode(currentWindow, GLFW.GLFW_CURSOR, GLFW.GLFW_CURSOR_NORMAL);
            } else if (visibilityState == CursorVisibility.HIDDEN && cursorMode == CursorMode.UNLOCKED) {
                GLFW.glfwSetInputMode(currentWindow, GLFW.GLFW_CURSOR, GLFW.GLFW_CURSOR_HIDDEN);
            } else if (visibilityState == CursorVisibility.HIDDEN && cursorMode == CursorMode.LOCKED) { 
                GLFW.glfwSetInputMode(currentWindow, GLFW.GLFW_CURSOR, GLFW.GLFW_CURSOR_DISABLED);
            }
        }

        public static void setCursorVisibility(CursorVisibility visibility) {visibilityState = visibility; updateCursor(); }
        public static CursorVisibility getCursorVisibility() {return visibilityState;}

        public static void setCursorMode(CursorMode mode) {cursorMode = mode; updateCursor(); }
        public static CursorMode getCursorMode() {return cursorMode;}
    }
